...
function ELiSecurity( $name){
    return str_replace( array( '-','#','/','，','|','、','\\','*','?','<','>','.',"\n","\r",'【','】','(',')','：','{','}','\'','"',':',';',' '), array('_',''), strtolower( trim($name)));
}
//Create a directory
...
class ELiDatabaseDriver{
    ...
    public function Safeconversion($data){
        if(!get_magic_quotes_gpc()) return ELiSql(addslashes($data));else return ELiSql($data);
    }
}
...
class ELiPdo extends ELiDatabaseDriver{ 
    ...
    public  function zhixing($moth='',$sql=''){ 
        ...
        if($moth=='scjg'){
            ...
            $onhouxu = ltrim( $onhouxu,$jsondate );
...
function ELivcode($sizes='1',$code="0123456789",$shu =4,$width=100,$height=50,$zadian=100,$xiaos = 6){
    ...
    $hhda = [];
    for( $i = 0 ; $i < $shu ; $i++ ){
        $hhda[] = $code{rand(0,strlen($code)-1)};
    }
    for( $i = 0 ; $i < $shu ; $i++ ){
        $hhda[] = $code1{rand(0,strlen($code1)-1)};
    }
    shuffle($hhda);
    $mkqian = implode('',$hhda);
...
class SubPages{     
    ...
    function subPageCss2(){
        ...
        if( $this -> current_page > 1){
            ...
            if(  PAGETRIM == 1)
                $dangqian = rtrim( $this->subPage_link, ELifenge);
        ...
        for( $i = 0 ; $i < count( $a ) ; $i++ ){
            ...
            if( $s == $this -> current_page ) $subPageCss2Str .= "<span class='hover'>". $s ."</span>";
                ...
                if($s < 2 && PAGETRIM == 1) $url = rtrim( $this->subPage_link, ELifenge).$this->houzui;
        ...
        if(PAGETRIM == 1 && $this->houzui != "" && $this->houzui != ELifenge){
...
//PAGETRIM  ELifenge
define( 'PAGETRIM', $ELiConfig['pagetrim']); 
...
if(!defined("Residentmemory")){
    ...
    ELiUri();
    $ELiHttp = ltrim( rawurldecode( trim($_SERVER["REQUEST_URI"])),'/');
