ELi['admin/integrallog'] = {
    type:{},
    hhhhda:"",
    init(){
        ELi['admin/integrallog'].get();
        ...
        layui.table.render({
        elem: '#user'+$class
        ,url: PLUG+'integrallog'+FENGE+TOKEN+FENGE+'get'
        ...
        $('.sopuliid'+$class).bind('input propertychange', function()
        {
          ...
          //逻辑
            ...
            $.post(PLUG+"integrallog"+FENGE+TOKEN+FENGE+'add',formdata,function(data){
              if(data.token && data.token != ""){
                TOKEN = data.token;
              }
              if(data.code == 1){
        ...
        layui.table.on('toolbar(user'+$class+')', function(obj){
            if(obj.event === 'add'){
              ELi['admin/integrallog'].add();
            }else if(obj.event === 'refresh'){
              layer.closeAll();
              ELi['admin/integrallog'].init();
        ...
        layui.table.on('row(user'+$class+')', function(obj){
          ELi['admin/integrallog'].edit(obj);
        });
