ELi['admin/adminlog'] = {
    ...
    init(){
        ...
        ,cols: [[
            ...
            ,{field:'uid', title: '管理UID'}
            ,{field:'type', title: '类型',templet  :function(d){
             
              return '<span class="qfys'+d.type+'">'+ELi['admin/adminlog'].type[d.type]+'</span>';
            }}
        ...
        ,done: function(data, curr, count){
            ...
            if( data.type ){
                ...
                ELi['admin/adminlog'].type = data.type;
        ...
        layui.table.on('toolbar(user'+$class+')', function(obj){
            if(obj.event === 'add'){
              ELi['admin/adminlog'].add();
            }else if(obj.event === 'refresh'){
              layer.closeAll();
              ELi['admin/adminlog'].init();
        ...
        layui.table.on('row(user'+$class+')', function(obj){
          ELi['admin/adminlog'].edit(obj);
        });
        layui.table.on('tool(user'+$class+')', function(obj){
          if(obj.event === 'del'){
            layer.confirm('真的要删除吗?', function(index){
              ELi['admin/adminlog'].del(obj);
              layer.close(index);
            });
        ...
        layui.form.on('submit(tijiao'+$class+')', function(formdata){
          ...
          var zuhesou = {};
            ...
            ELi['admin/adminlog'].hhhhda = $('[name="so_type"]').val();
      ...
      var BIAO = Array(
        ...
        'uid($$)管理UID($$)textshow($$)($$)管理UID($$)'+D.uid
        ,'type($$)类型($$)selectshow($$)($$)ELi[\'admin/adminlog\'].type($$)'+D.type+'($$)'
        ...
        ,'ff'+$class+'($$) ($$)submit($$)($$)layer.close(OPID);ELi[\'admin/adminlog\'].init()($$)($$)tijiao',
      );
      for(var z in BIAO){
        html+=jsfrom(BIAO[z]);
      }
