ELi['admin/currencylog'] = {
    type:{},
    hhhhda:"",
    init(){
        ELi['admin/currencylog'].get();
        ...
        layui.table.render({
        elem: '#user'+$class
        ,url: PLUG+'currencylog'+FENGE+TOKEN+FENGE+'get'
        ...
        $('.sopuliid'+$class).bind('input propertychange', function()
        {
          ...
          //逻辑
            ...
            $.post(PLUG+"currencylog"+FENGE+TOKEN+FENGE+'add',formdata,function(data){
              if(data.token && data.token != ""){
                TOKEN = data.token;
              }
              if(data.code == 1){
        ...
        layui.table.on('toolbar(user'+$class+')', function(obj){
            if(obj.event === 'add'){
              ELi['admin/currencylog'].add();
            }else if(obj.event === 'refresh'){
              layer.closeAll();
              ELi['admin/currencylog'].init();
        ...
        layui.table.on('row(user'+$class+')', function(obj){
          ELi['admin/currencylog'].edit(obj);
        });
